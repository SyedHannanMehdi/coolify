...
final class SetWorkspace
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if ($request->query->has('workspace_id')) {
            $workspaceId = $request->query->getString('workspace_id');
    ...
    {
        return redirect()->to($request->fullUrlWithQuery(['workspace_id' => $workspaceId]));
    }
    ...
    private function persist(string $workspaceId): void
    {
        ...
        cookie()->queue('workspace_id', $workspaceId, 43_200);
    }
}
